...
class Configuration implements ConfigurationInterface
{
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
            ...
            ->scalarNode('source_path_prefix')->defaultValue('source')->end()
            ->scalarNode('cache')->defaultValue('%kernel.project_dir%/var/cache/glide')->end()
            ->scalarNode('cache_path_prefix')->defaultValue('cache')->end()
            ...
            ->scalarNode('watermarks_path_prefix')->defaultValue('watermarks')->end()
            ->enumNode('driver')
                ->values(['gd', 'imagick'])
                ->defaultValue('gd')
            ->end()
            ...
            ->arrayNode('defaults')
                ->addDefaultsIfNotSet()
                ->children()
                    ->integerNode('q')->defaultValue(90)->end()
                    ->enumNode('fm')->values(['jpg', 'png', 'gif', 'webp', 'auto'])->defaultValue('auto')->end()
                ->end()
            ->end()
            ->arrayNode('presets')
